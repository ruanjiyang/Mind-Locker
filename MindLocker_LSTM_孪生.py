...
from keras.layers import Dense, Dropout,Activation,regularizers,merge,Input,Flatten
from keras.layers import Embedding, LSTM,SimpleRNN, Reshape,Lambda
...
def create_base_network(input_shape):
    ...
    # 
    x= LSTM(64,activation='softsign',dropout=0.001,recurrent_dropout=0.001, stateful=False,return_sequences=True)(input)  #stateful=True,可以使得帧组之间产生关联。 记得要在fit时候，shuffle=True。
    x= LSTM(64,activation='softsign',dropout=0.001,recurrent_dropout=0.001, stateful=False,return_sequences=False)(x)  #stateful=True,可以使得帧组之间产生关联。 记得要在fit时候，shuffle=True。

    x = Dense(64, activation='relu',activity_regularizer=regularizers.l1(1e-7))(x)
    x = Dropout(0.4)(x)
...
def euclidean_distance(vects):
    ...
    print('euclidean_distance=', K.sqrt(K.maximum(sum_square, K.epsilon())))
...
def read_from_file(file_name):
    f = open(file_name, 'r') #这是用于参与训练的他人EEG
...
def action(): #此函数是控制整个采样，训练， 匹配测试的过程。
    ...
    if step==3:   # 匹配测试
       ...
       ############################设置初始偏差值 开始##############################################
        ...
        # final_score=0
        # max_score=0
        # test_result=np.zeros(int(total_EEG_data_number/total_EEG_number_for_test))
        # for test_times in range (int(total_EEG_data_number/total_EEG_number_for_test)):  #0,1,2,3....
        ...
        #     f.close()
        #     EEG_data_original = EEG_data_original.astype('float32') / 10     # minmax_normalized
        ...
        # matched_average_score=(sum(test_result)-max_score)/(int(total_EEG_data_number/total_EEG_number_for_test)-1)
        ...
        for times in range(test_times): #测试十次   
            ...
            EEG_data_for_test=EEG_data_for_test.astype('float32') / 20      # 归一化
            ...
            #print('test_result=',test_result[times])
        ...
        test_average=(final_score-max_score)/(test_times-1)
        print('==================现在测试出来的Median Score================',test_median)
        print('==================现在测试出来的Average Score=================',test_average )
        #matched_median_score   matched_average_score
        #test_score_for_display=min((1-(abs(np.median(test_result)-matched_median_score))/matched_median_score)*50 + (1-(abs((final_score-max_score)/(test_times-1)-matched_average_score)/matched_average_score)*50),100)
    ...
    if step==2:  #机器学习
        ...
        if directly_load_EEG_flag== True:
            # f = open(directly_load_filename, 'r')  #读取代训练的EEG数据
            EEG_data_A=read_from_file(directly_load_filename)
            print("使用的是预先录取好的EEG文件:",directly_load_filename)
        if directly_load_EEG_flag== False:
            # f = open(filename+'EEG.txt', 'r')  #读取代训练的EEG数据
            EEG_data_A=read_from_file(filename+'EEG.txt')
            print("使用的是刚才录取的EEG文件:",filename)
        ###############开始读取B/C/D  ######################
        EEG_data_B=read_from_file('ljz_LSTM_2EEG.txt')
        EEG_data_C=read_from_file('2_EEG.txt')
        EEG_data_D=read_from_file('rjh_LSTM_EEG.txt')   #测试用： rjh_LSTM_EEG.txt  rjy_encoder_EEG.txt
        ...
        EEG_data_A = EEG_data_A.reshape((-1, time_steps, EEG_data_A.shape[1])) /10
        EEG_data_B = EEG_data_B.reshape((-1, time_steps, EEG_data_B.shape[1])) /10
        EEG_data_C = EEG_data_C.reshape((-1, time_steps, EEG_data_C.shape[1])) /10
        EEG_data_D = EEG_data_D.reshape((-1, time_steps, EEG_data_D.shape[1])) /10
        input_shape = EEG_data_A.shape[1:]
        ...
        # ##############我的改装##########################3
        # diff = keras.layers.concatenate([processed_a,processed_b])
        # out = Dense(18,activation='relu')(diff)
        # out = Dense(1,activation='sigmoid')(out)
        # model = Model([input_a,input_b],out)
        # ##############我的改装##########################3      


        distance = Lambda(euclidean_distance,output_shape=eucl_dist_output_shape)([processed_a, processed_b])  #原来的
        model = Model([input_a, input_b], distance) #原来的

        ##上面设置了孪生网络###
        rms = RMSprop()  #原
        model.compile(loss=contrastive_loss, optimizer=rms, metrics=[accuracy]) #原
        ...
        ########################开始训练#############################
        x1=np.concatenate((EEG_data_A,EEG_data_B),axis=2)    # A+B组合成正样本
        x2=np.concatenate((EEG_data_A,EEG_data_C),axis=2)    #A+D组合成负样本
        x=np.concatenate((x1,x2),axis=0)  #组合在一起: 正+负样本
        y1=np.zeros([int(total_EEG_data_number/time_steps),1])    #正样本对，标签为0.
        y2=np.ones([int(total_EEG_data_number/time_steps),1]) #负样本对，标签为1.
        y=np.vstack((y1,y2))
        ...
        ################检查 输入的shape##########
        print('len(x)=',len(x))
        print('EEG_data_A shape=',EEG_data_A.shape)
        print('EEG_data_B shape=',EEG_data_B.shape)
        ...
        print("开始训练")
        early_stopping = EarlyStopping(monitor='loss',patience=int(training_times*0.05),verbose=1,mode='min') 
        training_process_bar=Training_process_bar()
        model.fit([x[:,:, :18], x[:,:, 18:]], y, batch_size=training_batch_size,epochs=training_times,validation_split=0.33,callbacks=[early_stopping,training_process_bar])    
        model.save(filename+'_key.h5')
        print("把训练好的神经网络保存在:",filename,'_key.h5')


        print("开始测试下")
        test_data_same=np.concatenate((EEG_data_B,EEG_data_A),axis=2)  # 组合成正样本
        test_data_not_same=np.concatenate((EEG_data_B,EEG_data_C),axis=2) #组合成负样本


        print('test_data_same shape=',test_data_same.shape)
        print('test_data_not_same shape=',test_data_not_same.shape)
        test1=model.predict([test_data_same[:,:,:18], test_data_same[:,:,18:]])
        print ('测试看看！相同的EEG:   ', test1 )
        test2=model.predict([test_data_not_same[:,:,:18], test_data_not_same[:,:,18:]])
        print ('测试看看！不同的EEG:   ', test2 )
        print('test 1总数为',sum(test1))
        print('test 2总数为',sum(test2))
        ########################训练完毕，并保存训练好的神经网络#############################
...
def apply_parameters():
    ...
    total_EEG_data_number=int(max((int(ui.horizontalSlider.value()*total_EEG_data_number_times/time_steps))*time_steps,time_steps)) #读取n条EEG数据,必须是time_steps的整数。
