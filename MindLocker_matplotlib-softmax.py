...
def action(): #此函数是控制整个采样，训练， 匹配测试的过程。
    ...
    if step==3:   # 匹配测试
        ...
        for times in range(int(temp_total_EEG_group_number_for_test)): #这个大循环，内嵌一个1的小循环。     
            ...
            print("softmax test shape ==",test.shape)
            print("softmax test value ==",test )
    ...
    if step==2:  #机器学习
        ...
        y0 = np.ones([int(total_EEG_data_number/time_steps),4])               # class0 y data (tensor), shape=(100, 1)

        y1 = np.zeros([int(total_EEG_data_number/time_steps),4])                # class1 y data (tensor), shape=(100, 1)
        y2 = np.zeros([int(total_EEG_data_number/time_steps),4]) 
        y3 = np.zeros([int(total_EEG_data_number/time_steps),4])               # class1 y data (tensor), shape=(100, 1)
        ...
        # print("one X example", x[0][0])
        # print("one Y example",y[0])
        #手动 打乱 x,y 次序。
        ...
        model.add(LSTM(100,activation='tanh',input_shape = (time_steps, total_EEG_Features),dropout=0.0,recurrent_dropout=0.0, stateful = False, return_sequences=))  #stateful=True,可以使得帧组之间产生关联。 记得要在fit时候，shuffle=True。
        # model.add(Dense(100,kernel_regularizer=regularizers.l2(0.02),bias_regularizer=regularizers.l2(0.02)))
