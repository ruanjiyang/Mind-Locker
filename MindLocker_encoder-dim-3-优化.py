...
time_steps=1  #明天写注释！！！！
# 压缩特征维度
encoding_dim =  3  #1肯定不行， 2目前很好（但是对于rjy-C2不行，会出现高匹配）。 3似乎目前是最好的（对 rjy-C2也没问题）/3似乎也不行了 
...
#############################设置初始偏差值 结束##############################################
    ...
    if step==2:  #机器学习
        ...
        # 编码层 
        encoded = Dense(96, activation='relu',activity_regularizer=regularizers.l1(0))(input_img)  #10e-7
        encoded = Dense(48, activation='relu',activity_regularizer=regularizers.l1(0))(encoded)
        encoded = Dense(24, activation='relu',activity_regularizer=regularizers.l1(0))(encoded)
        encoded = Dense(12, activation='relu',activity_regularizer=regularizers.l1(0))(encoded)
        encoded = Dense(6, activation='relu',activity_regularizer=regularizers.l1(0))(encoded)
        encoder_output = Dense(encoding_dim)(encoded)
        
        # 解码层
        decoded = Dense(6, activation='relu',activity_regularizer=regularizers.l1(0))(encoder_output)
        decoded = Dense(12, activation='relu',activity_regularizer=regularizers.l1(0))(decoded)
        decoded = Dense(24, activation='relu',activity_regularizer=regularizers.l1(0))(decoded)
        decoded = Dense(48, activation='relu',activity_regularizer=regularizers.l1(0))(decoded)
        decoded = Dense(96, activation='relu',activity_regularizer=regularizers.l1(0))(decoded)
