...
def action(): #此函数是控制整个采样，训练， 匹配测试的过程。
    ...
    if step==3:   # 匹配测试
        ...
        for times in range(int(temp_total_EEG_data_number_for_test)): #这个大循环，内嵌一个1的小循环。     
    ...
    if step==2:  #机器学习
        ...
        y2 = np.zeros([int(total_EEG_data_number/time_steps),1])                # class1 y data (tensor), shape=(100, 1)
        x=np.vstack((EEG_data_A,EEG_data_B,EEG_data_C))
        print(x.shape)
        y = np.vstack((y0,y1,y2))   # shape (200,) LongTensor = 64-bit integer
        print(y.shape)
        #x=maxminnormal(x)  # 归一化x数据
        x = x.reshape((int(x.shape[0]/time_steps), time_steps, x.shape[1]))
        ########################开始搭建神经网络#############################
        model=Sequential()

        #model.add(Embedding(24,32,input_length=380))
        
        # input_shape 参数： smaples/timesteps/features
        #model.add(Reshape((1,24)))

        model.add(LSTM(100,activation='tanh',input_shape = (time_steps, total_EEG_Features),dropout=0.1,recurrent_dropout=0.1, return_sequences=False))
        #model.add(LSTM(100,activation='relu', dropout=0.2,recurrent_dropout=0.2, return_sequences=False))
        ...
        # model.add(Reshape((-1,24)))
        # model.add(SimpleRNN(240,activation='tanh',dropout=0.0,recurrent_dropout=0.0, return_sequences=False))
        ...
        ########################开始训练#############################
        #for i in range(1):  #这个for，是1次性训练，是为了在TensorBoard中查看训练情况。
        final_result_loss=""
        final_result_acc=""
        for i in range(100):  #这个for，只是为了进度条的显示，所以分成 100次来训练。
            per_step_result=model.fit(x, y, epochs=int(training_times/100), batch_size=training_batch_size,verbose = 1,shuffle=True) #这一行没有带callbacks，所以无法使用TensorBoard
            ...
            ui.progressBar.setProperty("value", i+2)
            QApplication.processEvents()  #用于PyQt界面的刷新，保证流畅程度。
            #history.loss_plot('epoch')
        #model.fit(x, y, epochs=training_times, batch_size=training_batch_size,verbose = 1,shuffle=True,callbacks=[tb]) #这一行带callbacks，是为了使用TensorBoard
...
def apply_parameters():
    ...
    total_EEG_Features=24  #这是固定的。每一条EEG数据都有24个参数值。
    training_batch_size=int(ui.horizontalSlider_2.value()*10)  #每次训练输入的EEG帧数
    training_times=int(ui.horizontalSlider_3.value()*20) #训练的次数
