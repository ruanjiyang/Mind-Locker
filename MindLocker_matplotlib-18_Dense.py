...
def action(): #此函数是控制整个采样，训练， 匹配测试的过程。
    ...
    if step==2:  #机器学习
        ...
        #     QApplication.processEvents()  #用于PyQt界面的刷新，保证流畅程度。
        early_stopping = EarlyStopping(monitor='val_loss',patience=int(training_times*0.1),verbose=1,mode='auto') 
        model.fit(x, y, validation_split=0.33,epochs=training_times, batch_size=training_batch_size,verbose = 1,shuffle=False,callbacks=[early_stopping,tb]) #这一行带callbacks，是为了使用TensorBoard
