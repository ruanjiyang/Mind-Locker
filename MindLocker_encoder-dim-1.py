...
time_steps=1  #明天写注释！！！！
# 压缩特征维度
encoding_dim =  2  #1肯定不行， 2目前很好（但是对于rjy-C2不行，会出现高匹配）。 3似乎目前是最好的（对 rjy-C2也没问题）/3似乎也不行了 
...
#############################设置初始偏差值 结束##############################################
    ...
    if step==2:  #机器学习
        ...
        f.close()
        EEG_data_B = EEG_data_B.astype('float32') / normal_number      # minmax_normalized
        ...
        # 编码层 
        encoded = Dense(64, activation='relu',activity_regularizer=regularizers.l1(0))(input_img)  #10e-7
        encoded = Dense(32, activation='relu',activity_regularizer=regularizers.l1(0))(encoded)
        encoded = Dense(16, activation='relu',activity_regularizer=regularizers.l1(0))(encoded)
        ...
        # 解码层
        decoded = Dense(4, activation='relu',activity_regularizer=regularizers.l1(0))(encoder_output)
        decoded = Dense(8, activation='relu',activity_regularizer=regularizers.l1(0))(decoded)
        decoded = Dense(16, activation='relu',activity_regularizer=regularizers.l1(0))(decoded)
        decoded = Dense(32, activation='relu',activity_regularizer=regularizers.l1(0))(decoded)
        decoded = Dense(64, activation='relu',activity_regularizer=regularizers.l1(0))(decoded)
        ...
        ########################开始训练#############################
        early_stopping = EarlyStopping(monitor='val_loss',patience=int(training_times*0.2),verbose=1,mode='min') 
        ...
        #autoencoder.fit(x_train, x_train, validation_split=0.33,epochs=training_times, batch_size=training_batch_size, shuffle=True,callbacks=[tb,early_stopping,training_process_bar])
