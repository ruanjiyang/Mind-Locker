...
#bug： 选择打开一个保存的model，但是又没真正打开，那么程序会出错。
...
total_EEG_data_number_times=40  #此参数设置了EEG采样的倍数。比如中间值为12*50,注意，因为两个陪训的数据集都只有2千条，所以采样的EEG也最多只能2千条，所以这个数字只能设置为最大19.
...
def Normalize(data):  #归一化数组？？
    # Zmax,Zmin=data.max(axis=0),data.median(axis=0)
...
#############################设置初始偏差值 开始##############################################
        ...
        matched_average_score=(final_score-max_score)/(int(total_EEG_data_number/total_EEG_number_for_test)-1)
        matched_median_score=np.median(test_result)
        print('==================Average Score================',matched_average_score)
        print('==================Median Score================',matched_median_score)
...
#############################设置初始偏差值 结束##############################################
        ...
        print('==================Median Score================', np.median(test_result))
        print('==================Average Score=================', (final_score-max_score)/(test_times-1))

        test_score_for_display=min(((1-(np.median(test_result)-matched_median_score)/matched_median_score)*50 + (1-((final_score-max_score)/(test_times-1)-matched_average_score)/matched_average_score)*50),100)
    ...
    if step==2:  #机器学习
       ...
       # 压缩特征维度至2维
        ...
        # 编码层
        encoded = Dense(12, activation='relu',activity_regularizer=regularizers.l1(0))(input_img)  #10e-7
        encoded = Dense(8, activation='relu',activity_regularizer=regularizers.l1(0))(encoded)
        encoded = Dense(4, activation='relu',activity_regularizer=regularizers.l1(0))(encoded)
        encoded = Dense(2, activation='relu',activity_regularizer=regularizers.l1(0))(encoded)
        # encoded = Dense(20, activation='relu',activity_regularizer=regularizers.l1(0))(encoded)

        encoder_output = Dense(encoding_dim)(encoded)
        
        # 解码层
        decoded = Dense(2, activation='relu',activity_regularizer=regularizers.l1(0))(encoder_output)

        decoded = Dense(4, activation='relu',activity_regularizer=regularizers.l1(0))(decoded)

        decoded = Dense(8, activation='relu',activity_regularizer=regularizers.l1(0))(decoded)
        decoded = Dense(12, activation='relu',activity_regularizer=regularizers.l1(0))(decoded)
        ...
        # training
        print(x_train.shape)
        early_stopping = EarlyStopping(monitor='loss',patience=int(training_times*0.01),verbose=1,mode='auto') 
        ########################神经网络搭建完毕#############################
        ...
        for i in range(training_loop_times):  #这个for，只是为了进度条的显示，所以分成 10次来训练。
            per_step_result=autoencoder.fit(x_train, x_train, epochs=int(max(training_times/training_loop_times,1)), batch_size=training_batch_size, shuffle=True,callbacks=[early_stopping])
            per_step_result_for_display=str(per_step_result.history['loss'][0])
            print("Training loop times:",i,"/100")
            ui.label.setText("开始机器学习你的脑纹,目前的损失率为："+per_step_result_for_display)
        ...
        x = encoder.predict(EEG_data_B.reshape(int(total_EEG_data_number/time_steps),int(time_steps*total_EEG_Features)))
...
def apply_parameters():
    ...
    total_EEG_number_for_test=int(max(ui.horizontalSlider_4.value(),1))
