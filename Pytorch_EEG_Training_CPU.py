...
net = torch.nn.Sequential(
    torch.nn.Linear(total_EEG_Features,100),
    torch.nn.ReLU(),
...
for t in range(training_times):
    ...
    if t % 2 == 0:
        ...
        for i in range(0,24,1):
            plt.scatter(x_zhou.data.numpy()[:, i], x.data.numpy()[:, i], c=pred_y, s=0.2+x.data.numpy()[:, i]*5, lw=1, cmap='RdYlGn',alpha=0.2)
        ...
        plt.text(5,np.max(x.data.numpy())-5, out_text,fontdict={'size': 20, 'color':  'red'})
        plt.pause(0.1)
...
# torch.save(net, 'ruanjiyang_net.pkl')

# ruanjiyang_test=torch.Tensor([2.0418024,0.0041489145,0.1542012,0.45853466,0.11114338,0.011107396,2.3692565,1.476879,3.1518886,2.923403,0.9873173,4.0479565,3.5091522,2.186714,0.06271454,0.32675493,0.55380744,0.111587375,0.030802552,0.17690378,0.82096994,1.40196,0.87992424,0.22100423])

# ruanminli_test=torch.Tensor([1.5566941,0.13937913,8.110901,8.8937025,1.4536568,0.2568266,2.3953876,4.215058,7.2890697,5.6187687,5.874292,2.7794783,2.5033157,1.6079297,0.33475968,2.1464062,2.1329226,0.40537274,0.14721768,0.7805021,4.4017987,4.501646,1.9571599,0.65970874])
...
# for k in range(total_EEG_data_number):
#     EEG_data_one_line=(All_EEG_data_lines[k+500].split('A'))  ####按照字符"A"来截断每一条EEG数据，分割成24小份
#     # print(EEG_data_one_line)
#     for i in range(total_EEG_Features):
#         EEG_data_A[k][i]=float(EEG_data_one_line[i])
# f.close()
